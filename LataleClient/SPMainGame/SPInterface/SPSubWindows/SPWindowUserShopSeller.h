...
#define WIID_USERSHOP_SELL_TOP			(WIID_USERSHOP_SELL	+	10)
#define WIID_USERSHOP_SELL_CENTER		(WIID_USERSHOP_SELL	+	20)

#define WIID_USERSHOP_SELL_TITLE		(WIID_USERSHOP_SELL	+	22)
#define WIID_USERSHOP_SELL_CLOSE		(WIID_USERSHOP_SELL	+	25)

#define WIID_USERSHOP_SELL_MIDDLE		(WIID_USERSHOP_SELL	+	30)
#define WIID_USERSHOP_SELL_NAME			(WIID_USERSHOP_SELL	+	33)
#define WIID_USERSHOP_SELL_NAME_EDIT	(WIID_USERSHOP_SELL	+	35)

#define WIID_USERSHOP_SELL_ITEM_SKIN	(WIID_USERSHOP_SELL	+	40)		//4개씩 상점 아이템 리스트 리스트 스킨
#define WIID_USERSHOP_SELL_ITEM_ENABLE	(WIID_USERSHOP_SELL +	40)		//4개씩 아이템이 있는 경우 추가 스킨
#define WIID_USERSHOP_SELL_ITEM_LINE	(WIID_USERSHOP_SELL	+	50)		//4개씩 상점 아이템 경계
#define WIID_USERShOP_SELL_ITEM_ICONLINE (WIID_USERSHOP_SELL	+	55) //4개씩 아이템 아이콘 영역 외곽 라인
#define WIID_USERSHOP_SELL_ITEM_ICON	(WIID_USERSHOP_SELL	+	60)		//4개씩 아이템 아이콘 
#define WIID_USERSHOP_SELL_ITEM_NAME	(WIID_USERSHOP_SELL	+	70)		//4개씩 아이템 명
#define WIID_USERSHOP_SELL_ITEM_EMPTY	(WIID_USERSHOP_SELL	+	80)		//4개씩 아이템 명이 없는 경우
#define	WIID_USERSHOP_SELL_ITEM_ELY		(WIID_USERSHOP_SELL	+	90)		//4개씩 아이템 가격
#define	WIID_USERSHOP_SELL_ITEM_ELY_ICON	(WIID_USERSHOP_SELL	+	100)
#define WIID_USERSHOP_SELL_ITEM_BUTTON	(WIID_USERSHOP_SELL	+	110)	//4개씩 아이템 내부 버튼

#define WIID_USERSHOP_SELL_PREV			(WIID_USERSHOP_SELL	+	140)	//
#define WIID_USERSHOP_SELL_PAGE			(WIID_USERSHOP_SELL	+	145)	//
#define WIID_USERSHOP_SELL_NEXT			(WIID_USERSHOP_SELL	+	150)	//

#define WIID_USERSHOP_SELL_OK			(WIID_USERSHOP_SELL	+	160)	//
#define WIID_USERSHOP_SELL_CANCEL		(WIID_USERSHOP_SELL	+	165)	//

#define WIID_USERSHOP_SELL_BOTTOM		(WIID_USERSHOP_SELL	+	170)


const int MAX_USER_SHOP_LIST = 4;			//한 페이지에 보이는 아이템의 갯수
const int MAX_ITEM_SIZE		= 16;			//판매 리스트 사이즈 

/**
 * <br>
*/
enum USERSHOP_STATE {
	USERSHOP_STATE_NULL = 0,				/**< 아무런 상태도 아님 <br> */
	USERSHOP_STATE_CREATE = 1,				/**< 상점 개설 준비 상태 <br> */
	USERSHOP_STATE_CREATE_REQUEST = 2,		/**< 상점 개설 요청 <br> */
	USERSHOP_STATE_SELL = 3,				/**< 상점 개설 요청의 응답으로 실제 상점 모드 <br> */
};

class SPWindowUserShopSeller : public SPWindow 
{
public:
	SPWindowUserShopSeller( INSTANCE_ID InstanceID, int iX, int iY, int iCX, int iCY, SPWindow* pParent = NULL);
	virtual ~SPWindowUserShopSeller();
...
private:
	...
	USERSHOP_STATE	m_iUserShopState;							/**< 현재 개인 상점 상황 <br> */
	...
	int				m_iUserShopType;							/**< 내 상점 타입 <br> */
