...
#define RETSZ_HotQTopNV2_OK			0x1c10
#define RETSZ_HotQTopNV2_NOMATCH	0x1c15




//---------------------------------------------------------------------
// RS Menu Data Structurre
struct RS_MENU
{
	int		iCount;
	char	szAC[10][16] ;
...
struct RS_ANSWER
{
	char	szMasterAns[2560];
	char	szLogIdx[64];
	int		iOtherCount ;
	char	szOtherAns[10][256] ;
	RS_ANSWER()
	{
		Init() ;
	}
	...
	{
		iOtherCount = 0 ;
		memset(szMasterAns, 0, 2560);
		memset(szLogIdx, 0, 64) ;
		for(int i = 0; i < 10; i++ )	{
			memset(szOtherAns[i], 0, 256);
		}
	}
};